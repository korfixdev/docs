...
- [ ] **Права в `access_db` прописаны** для нужных ролей:
    ...
    - Если миниап должны использовать другие роли (менеджер, оператор, клиент) — обновить `access_db` с соответствующими `acctype_*` (см. `self-provisioning.md`)
    - Либо явно в `about` раздел «Настройка» написать инструкцию админу прописать права после установки
