...
1. Открой маркетплейс в интерфейсе: `/db/marketplace`
2. Найди своё приложение (по `name` из `config.json`)
3. Нажми кнопку **Установить** на карточке приложения
4. Фреймы появятся в указанных в `config.json` точках встраивания

> Реестр `installed_apps` заполняется автоматически — туда писать руками не нужно. Просмотреть установленные приложения можно в `/db/installed_apps`.
