...
```js
// Авторизация по токену — для ВНЕШНИХ вызовов (curl / сервер / n8n), см. примеры с Bearer/curl ниже.
// Никогда не хардкодь токен платформы в шипнутом миниапе (FAIL на ревью маркетплейса).

// /api/user/tariff — тариф и биллинговая инфа текущего пользователя (из апа работает по сессии, без токена)
...
> ℹ️ **Внутри миниапа используй `/db/...` (с `form[]`) и не передавай токен.** `App.fetch`
> проксирует запрос через залогиненное родительское окно, поэтому авторизация идёт по **сессии** —
> токен не нужен, и хардкодить токен платформы в шипнутом миниапе нельзя (FAIL на ревью).
> Путь с Bearer-токеном (`/api/db/...` + `Authorization: Bearer`) — для **внешних** вызовов:
> curl / CI / сервер / n8n.
...
### HTTP-коды для write-запросов через `/api/db/`

`POST /api/db/{catalog}` и `POST /api/db/{catalog}/{id}` возвращают правильные HTTP-коды:

| Сценарий | HTTP-код | `ok` |
|----------|----------|------|
| Запись создана | **201 Created** | `true` |
| Запись обновлена | **200 OK** | `true` |
| Ошибка валидации (не заполнено обязательное поле и т.п.) | **422** | `false` |
| Запись не найдена (update по несуществующему id) | **404** | `false` |
| Серверная ошибка | **500** | `false` |

Все ответы теперь содержат поле `ok: boolean` в теле:
...
> **Эндпоинты `/db/` (сессия) не изменились** — они по-прежнему возвращают HTTP 200 с `status: 'error'` в теле, чтобы не сломать существующий UI.

В новом коде используй `App.fetchV2()` — он читает `ok` и нормализует форму ответа.
