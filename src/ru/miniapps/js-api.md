...
| `App.storage.getValue(key, default?)` | Promise | Голое значение + default (рекомендуется) |
| `App.storage.get(key)` | Promise | Запись целиком `{name, value, alias, ...}` — читай `.value` |
| `App.storage.getRow(key)` | Promise | Явный алиас `get()` |
