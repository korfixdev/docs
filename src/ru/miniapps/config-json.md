...
> Все поля метаданных ниже с пометкой **да** — обязательны. Локальный пре-деплой гейт
> (`validate-bundle.js`), скилл `korfix-miniapp-validate` и `schemas/config.schema.json`
> требуют один и тот же набор — бандл, прошедший один гейт, проходит и остальные.
...
| `package` | да | Имя пакета (имя папки приложения). Уникальный идентификатор |
| `category` | да | Категория маркетплейса, целое 1..5: 1 ИИ-агенты, 2 Бизнес, 3 Игры, 4 Инструменты, 5 Прочее |
| `logo` | да | Имя файла лого из zip. SVG предпочтителен |
| `urls` | да | Словарь фреймов. Ключ → путь. Для локальных — относительный, для remote — абсолютный. **`urls.main` обязателен если приложение должно открываться из меню или через кнопку «Установить»** |
| `permissions` | да | Декларация прав доступа (см. ниже) |
| `tags` | опционально | Теги через запятую. Используются для фильтрации в маркетплейсе |
...
- **Ключ** = alias каталога в меню, **ПОСЛЕ которого** вставляется пункт. Например `"tt_tasks"` — сразу за разделом «Задачи», `"installed_apps"` — после раздела «Приложения».
- **Значение**: `{ "frame": "имя_из_urls", "name": "Подпись" }`.
- Несколько пунктов — несколько ключей.
- Если приложение является виджетом дашборда или встраивается только в каталоги — блок `menu` не нужен.
- **`urls.main` обязателен**: без него платформа не знает что открывать при клике на пункт меню. Приложение без `urls.main` не отображается в UI маркетплейса как «доступное для установки».
...
Платформа автоматически записывает `category` в БД при первом install приложения (если поле ещё пусто). Дальнейшие правки — через UI каталога `/db/marketplace`, и они **не перетираются** при повторном deploy.
