...
#### Через API — таблица выбора эндпоинта (канон)

Это **единственный источник правды** по тому, какой деплой-эндпоинт вызывать. Всё остальное
(агенты, скиллы) ссылается сюда.

| Цель | Эндпоинт (POST) | Примечания |
|------|-----------------|------------|
| **Создать** новое приложение (ID ещё нет) | `/api/db/marketplace` (алиас `/api/db/marketplace/add`) + `-F name=...` | Вернёт `{"id","alias"}`. `deploy/` создавать **не умеет**. |
| **Обновить** существующее (по умолчанию) | `/api/db/marketplace/{ID}` | Стандартный путь. Загружает zip + уведомляет store внутренним хуком. В проде достаточно. |
| **Обновить + принудительно сбросить `appconfig`** | `/api/marketplace/deploy/{ID}` | Алиас = update + refresh в одном вызове. Нужен существующий ID. Когда надо явно сбросить кеш конфига. |
| **Только инвалидация кеша** | `/api/marketplace/refresh/{ID}` | Когда zip уже загружен, а кеш устарел. |

Оба update-эндпоинта (`/api/db/marketplace/{ID}` и `/api/marketplace/deploy/{ID}`) прогоняют
одинаковую manifest-валидацию при деплое (errors → `422` блок, warnings → успех). Авторизация на
всех: заголовок `Authorization: Bearer {TOKEN}` **или** query `?token={TOKEN}`.

> **По умолчанию для обновлений — `/api/db/marketplace/{ID}`.** Бери `/api/marketplace/deploy/{ID}`
> только когда нужен сброс кеша одним запросом.
